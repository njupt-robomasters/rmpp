...
void Fuzzy_Rule_Implementation(FuzzyRule_t *fuzzyRule, float measure, float ref) {
    ...
    fuzzyRule->dt = BSP_DWT_GetDeltaT((void *) fuzzyRule->DWT_CNT);
    ...
    eLeftTemp = fuzzyRule->e >= 3 * fuzzyRule->eStep
                    ? 0
                    : (fuzzyRule->e <= -3 * fuzzyRule->eStep ? 1 : (eRightIndex - fuzzyRule->e / fuzzyRule->eStep - 3));
    eRightTemp = fuzzyRule->e >= 3 * fuzzyRule->eStep
                     ? 1
                     : (fuzzyRule->e <= -3 * fuzzyRule->eStep ? 0 : (fuzzyRule->e / fuzzyRule->eStep - eLeftIndex + 3));
...
float PID_Calculate(PID_t *pid, float measure, float ref) {
    ...
    pid->dt = BSP_DWT_GetDeltaT((void *) &pid->DWT_CNT);
...
float Feedforward_Calculate(Feedforward_t *ffc, float ref) {
    ffc->dt = BSP_DWT_GetDeltaT((void *) &ffc->DWT_CNT);
...
float LDOB_Calculate(LDOB_t *ldob, float measure, float u) {
    ldob->dt = BSP_DWT_GetDeltaT((void *) &ldob->DWT_CNT);
    ...
    ldob->Disturbance = ldob->c[0] * ldob->Measure + ldob->c[1] * ldob->Measure_dot + ldob->c[2] * ldob->Measure_ddot -
                        ldob->u;
...
float TD_Calculate(TD_t *td, float input) {
    ...
    td->dt = BSP_DWT_GetDeltaT((void *) &td->DWT_CNT);
