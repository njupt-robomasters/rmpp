...
public:
    // 电机状态
    float angle = 0; // 转子角度（减速前）【单位：角度】
    float speed_rpm = 0, speed_rpm_lpf = 0; // 转速（减速后）【单位：rpm】
    float speed_tps = 0, speed_tps_lpf = 0; // 转速（减速后）【单位：圈/s】
    float speed_aps = 0, speed_aps_lpf = 0; // 转速（减速后）【单位：角度/s】
...
private:
    ...
    int16_t speed_rpm_raw = 0; // 转速（减速前）【单位：rpm】
