#pragma once

#include "app.h"

#include "cmsis_os.h"

#include "bsp_led.h"
#include "bsp_dwt.h"
#include "bsp_can.h"
#include "bsp_uart.h"
#include "bsp_cdc.h"
#include "bsp_pwm.h"

#include "dj6.hpp"
#include "chassis_4wheel.hpp"
#include "imu.hpp"
#include "module/gimbal.hpp"
#include "referee.hpp"
#include "ui.hpp"

#include "task/task_led.h"
#include "task/task_chassis.h"
#include "task/task_imu.h"
#include "task/task_gimbal.h"
#include "task/task_protocol.h"

class Settings {
public:
    // 底盘
    PID::param_t wheel_pid = {.kp = 10000.0f, .ki = 50.0f};
    float wheel_radius = 0.063f; // 轮子半径【单位：m】
    float chassis_radius = 0.225f; // 底盘半径【单位：m】

    // 云台
    float pitch_aps_max = 60; // 【单位：度/s】
    float yaw_aps_max = 180; // 【单位：度/s】
    float mouse_x_max = 300;
    float mouse_y_max = 300;
    IMU::param_t imu_param = {.yaw = 0, .pitch = 0, .roll = 180};
    PID::param_t pitch_pid = {.kp = 1.4f, .kd = 0, .ff = 0.75f}; // pitch
    PID::param_t yaw_pid = {.kp = 0.7f, .kd = 0.012f}; // yaw
    PID::param_t shoot_pid = {.kp = 0.035f, .kd = 0.014f, .ff = -0.8f}; // shoot
};

class Status {
public:
    // 底盘
    float chassis_vxy_max = 2.0f; // 移动最大速度【单位：m/s】
    float chassis_avy = 1.0f; // 移动加速度【单位：m/s^2】
    float chassis_vr_rpm = 60.0f; // 小陀螺旋转速度【单位：rpm】

    // 云台
    Gimbal::mode_e gimbal_mode = Gimbal::IMU_MODE;
    bool is_prepare_shoot = false;
    bool is_shoot = false;
    bool is_rv2_mode = false;
    float shoot_freq = 15.0f; // 【单位：Hz】
};

extern Settings settings;
extern Status status;
extern DJ6 dj6;
extern Chassis4Wheel chassis;
extern IMU imu;
extern Gimbal gimbal;
extern Referee referee;
extern UI ui;
