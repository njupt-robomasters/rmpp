...
#define MAVLINK_MSG_ID_imu_MIN_LEN 12
#define MAVLINK_MSG_ID_0_LEN 12
#define MAVLINK_MSG_ID_0_MIN_LEN 12

#define MAVLINK_MSG_ID_imu_CRC 178
#define MAVLINK_MSG_ID_0_CRC 178
...
#define MAVLINK_MESSAGE_INFO_imu { \
    ...
    "imu", \
    3, \
    {  { "yaw", NULL, MAVLINK_TYPE_FLOAT, 0, 0, offsetof(mavlink_imu_t, yaw) }, \
         { "pitch", NULL, MAVLINK_TYPE_FLOAT, 0, 4, offsetof(mavlink_imu_t, pitch) }, \
         { "roll", NULL, MAVLINK_TYPE_FLOAT, 0, 8, offsetof(mavlink_imu_t, roll) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_imu { \
                               ...
                               float yaw, float pitch, float roll)
{
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_imu_MIN_LEN, MAVLINK_MSG_ID_imu_LEN, MAVLINK_MSG_ID_imu_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_imu_MIN_LEN, MAVLINK_MSG_ID_imu_LEN);
#endif
}
...
{
    return mavlink_msg_imu_pack(system_id, component_id, msg, imu->yaw, imu->pitch, imu->roll);
}
...
{
    return mavlink_msg_imu_pack_chan(system_id, component_id, chan, msg, imu->yaw, imu->pitch, imu->roll);
}

/**
 * @brief Encode a imu struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param imu C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_imu_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_imu_t* imu)
{
    return mavlink_msg_imu_pack_status(system_id, component_id, _status, msg,  imu->yaw, imu->pitch, imu->roll);
}
...
static inline void mavlink_msg_imu_send(mavlink_channel_t chan, float yaw, float pitch, float roll)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_imu_LEN];
    _mav_put_float(buf, 0, yaw);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_imu_send(chan, imu->yaw, imu->pitch, imu->roll);
#else
 ...
 */
static inline void mavlink_msg_imu_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  float yaw, float pitch, float roll)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
